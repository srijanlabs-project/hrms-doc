...
| AuthN/AuthZ | **Phase 3 (shipped):** OTP-only login in NestJS, no passwords — `OtpChallenge` table (bcrypt-hashed code, expiry, attempt cap) behind an `OtpProvider` interface; `StaticDevOtpProvider` returns a fixed dev code (`123456`), logged and echoed in the API response since it never varies. DB-backed sessions (revocable, not purely stateless), JWT cookie carrying the session id. RBAC v1: a plain string-array `roles` column, `@Roles()`/`RolesGuard` gating. **Target (deferred):** a real SMS/email OTP gateway before UAT (swap `OtpProvider`'s implementation only — verify/session/RBAC code is untouched), and Keycloak (OIDC) for SSO/MFA/federation once Docker or a hosted IdP is available — see §7. | Local OTP auth is an explicitly sanctioned pattern in `08-.../03-identity-and-access/01-authentication.md` §3 ("magic-link... approved enterprise authentication pattern"); building it now doesn't abandon Keycloak, it implements the spec's own interim allowance while federation is blocked on infrastructure |
...
- UAT approaches — swap `StaticDevOtpProvider` for a real SMS/email gateway (`OtpProvider` in `apps/api/src/auth/otp/otp-provider.ts`); no other auth code changes
- Docker (or a hosted Keycloak instance) becomes available, or a customer requires real SSO/MFA/SAML — swap the AuthGuard for OIDC token verification against Keycloak; the session/role model (User.roles, RequestContextService, RolesGuard) does not need to change, only how a session is established
